...
def automate_ui_import(host, email, password, zipped_files):
    ...
    with sync_playwright() as p:
        ...
        # 2. Go to Fragments Page (Moved inside the loop for sequential robustness)
        ...
        for zip_path in zipped_files:
            ...
            print(f"Navigating to Page Fragments Panel for [{collection_name}]...")
            ...
            try:
                ...
                if sheet_el.count() > 0:
                    ...
                    if is_warning:
                        ...
                        print("\n".join(summary_items))
                        print("=" * 80 + "\n")
                        pass # patched short-circuit bug
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Liferay Page Fragments Deployer Script")
    parser.add_argument('--host', help="Liferay DXP host URL")
    parser.add_argument('--email', help="Admin email prefix/address")
    parser.add_argument('--password', help="Admin password")
    parser.add_argument('--dry-run', action='store_true', help="Package and validate collections but skip browser deployment")
    args = parser.parse_args()
    
    fragments_dir = get_fragments_dir()
    if not fragments_dir:
        print("Error: Could not find 'liferay/fragments/' directory. Run from project root.", file=sys.stderr)
        sys.exit(1)
    ...
    if not success or not zipped_files:
        print("\nError: Packaging failed.", file=sys.stderr)
        sys.exit(1)
